...
export const OrderSummarySection: React.FC<OrderSummarySectionProps> = ({ onPlaceOrder, isLoading }) => {
  ...
  return (
    <div className="bg-gray-50 p-8 rounded-3xl border border-gray-200 sticky top-24">
        ...
        <ul className="space-y-6 mb-8 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {items.map(item => (
                <li key={item.id} className="flex gap-4">
                    <div className="h-16 w-16 rounded-md overflow-hidden border border-gray-200 bg-white flex-shrink-0">
                    ...
                    <div className="flex-1">
                            ...
                            <div className="text-xs text-gray-500 mt-1">Qty: {item.quantity} × ${item.price}</div>
                    </div>
                </li>
            ))}
        </ul>
        ...
        <p className="text-xs text-center text-gray-400 mt-4">
            Secure Checkout powered by Wompi.
        </p>
    </div>
  );
};
